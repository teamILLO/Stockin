...
class UsersTestCase(TestCase):
    ...
    def test_signin(self):
        ...
        response = client.post('/api/users/signin/', json.dumps(
            {'invalid': 'normal@user.com'}), content_type='application/json', HTTP_X_CSRFTOKEN=csrftoken)
    ...
    def test_send_code(self):
        ...
        response = client.post('/api/users/sendCode/', json.dumps(
            {'email': 'normal@normal.com', 'code' : '1'}), content_type='application/json', HTTP_X_CSRFTOKEN=csrftoken)
        self.assertEqual(response.status_code, 204)
    ...
    def test_user_info(self):
        ...
        csrftoken = response.cookies['csrftoken'].value
        response = client.put('/api/users/userInfo/', json.dumps(
            {'change': 'nickname', 'email': 'foo@foo.com', 'nickname': 'foo_new'}), content_type='application/json', HTTP_X_CSRFTOKEN=csrftoken)
        self.assertEqual(response.status_code, 404)

        response = client.get('/api/users/token/')
        csrftoken = response.cookies['csrftoken'].value
        response = client.put('/api/users/userInfo/', json.dumps(
            {'change': 'nickname', 'email': 'normal@normal.com', 'nickname': 'foo_new'}), content_type='application/json', HTTP_X_CSRFTOKEN=csrftoken)
        self.assertEqual(response.status_code, 201)
