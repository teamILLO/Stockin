...
def build_dataset(time_series, seq_length):
    dataX=[]
    dataY=[]
    for i in range(80, len(time_series) - seq_length):
        robustScaler.fit(time_series[i-80: i+seq_length])
        After_normalization = robustScaler.transform(time_series[i-80:i+seq_length])
        _x = After_normalization[80:80 +seq_length, :]
...
try:
    net.load_state_dict(torch.load('./Models/20day_models.pt'))
except:
    pass
...
torch.save(net.state_dict(),'./Models/20day_models.pt')
