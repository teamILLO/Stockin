...
describe('<SignupModal />', () => {
  ...
  test(`should show error on invalid email input`, () => {
    render(signup);
    fireEvent.click(screen.getAllByText(/trigger/i, { selector: 'button' })[0]);
    const query1 = screen.getByPlaceholderText('Email');
    fireEvent.change(query1, { target: { value: 'a' } });
    const query2 = screen.getByRole('button', { name: /next/i });
    fireEvent.click(query2);
    const query3 = screen.queryAllByRole('alert');
    expect(query3.length).toBe(1);
  });

  test(`should change value of email on invalid email input`, () => {
    render(signup);
    fireEvent.click(screen.getAllByText(/trigger/i, { selector: 'button' })[0]);
    const query1 = screen.getByPlaceholderText('Email');
    fireEvent.change(query1, { target: { value: 'a' } });
    const query2 = screen.getByRole('button', { name: /next/i });
    fireEvent.click(query2);
    const query3 = screen.getByPlaceholderText('Email');
    fireEvent.change(query3, { target: { value: 'test@email.com' } });
    expect(query3.value).toBe('test@email.com');
  });

  test(`should change value of password on invalid email input`, () => {
    render(signup);
    fireEvent.click(screen.getByText(/trigger/i, { selector: 'button' }));
    const query1 = screen.getByPlaceholderText('Email');
    fireEvent.change(query1, { target: { value: 'a' } });
    const query2 = screen.getByRole('button', { name: /next/i });
    fireEvent.click(query2);
    const query3 = screen.getByPlaceholderText('Password');
    fireEvent.change(query3, { target: { value: 'password' } });
    expect(query3.value).toBe('password');
  });
  ...
  test(`should change value in nickname input field`, async () => {
    ...
    await wait(() => fireEvent.click(screen.getByText(/Next/i, { selector: 'button' })));
  ...
  test(`should show alert when duplicated nickname`, async () => {
    ...
    await wait(() => fireEvent.click(screen.getByText(/sign up/i, { selector: 'button' })));
    const query = screen.getByPlaceholderText('Nickname');
    fireEvent.change(query, { target: { value: 'nickname' } });
    expect(query.value).toBe('nickname');
  });
